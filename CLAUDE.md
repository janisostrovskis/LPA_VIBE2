...
- **New frontend pages always require a parallel i18n-agent handoff.** `frontend/public/locales/` is i18n-Agent scope — frontend-agent must NOT write locale JSON files. Any sub-phase plan that ships new UI pages must dispatch i18n-agent in the same batch (or immediately after) to add keys for all three locales. Frontend-agent notes missing keys in its handoff Notes; i18n-agent delivers them. Why: Phase 02f — auth pages shipped without translation keys; all three locales required post-dispatch fixup by main session.
